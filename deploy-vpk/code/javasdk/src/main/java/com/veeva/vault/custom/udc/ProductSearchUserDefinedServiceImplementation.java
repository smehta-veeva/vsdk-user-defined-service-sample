...
public class ProductSearchUserDefinedServiceImplementation implements ProductSearchUserDefinedService {
    ...
    public Map<String, BicyclePartData> doesProductExist(Map<String, String> productAndManufacturerMap) {
        ...
        String partAndManufacturerQueryString = "SELECT id, name__v, quantity__c FROM bicycle_part__c WHERE caseinsensitive(name__v) CONTAINS ("
                + productsToQuery + ") AND caseinsensitive(bicycle_part_manufacturer__cr.name__v) CONTAINS (" + manufacturersToQuery + ")";

        // Running our query and getting the response.
        QueryResponse queryResponse = queryService.query(partAndManufacturerQueryString);
